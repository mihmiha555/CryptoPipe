...
static unsigned int F(struct BLOWFISH_CTX *ctx, unsigned int x)
{
...
void blowfish_init(struct BLOWFISH_CTX *ctx, unsigned char *key, int key_size)
{
    ...
    for(i = 0, k = 0; i < N + 2; i++)
    {
        for(j = 0, long_key = 0; j < 4; j++, k++)
            long_key = (long_key << 8)| key[k % key_size]; 
        ctx->p[i] = ORIG_P[i] ^ long_key;
    }
    ...
    for (i = 0; i < N + 2; i += 2)
    {
        blowfish_encrypt_block(ctx, &xl, &xr);
        ctx->p[i] = xl;
        ctx->p[i + 1] = xr;
    }
